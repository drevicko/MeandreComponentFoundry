 ...
 * 
 * <p>
 * Count maps have a key representing items to count and java.lang.Number values
 * as the counts. In many cases, the Number values are of type
 * java.lang.Integer, but this is not necessarily the case. For example, a count
 * map may have strings as keys and java.lang.Double scaled word frequencies as
 * values.
 * </p>
 */
...
public class CountMapUtils {
	 ...
	 * 
	 * @param map
	 *            The map with string keys and Number counts as values.
	 * 
	 * @return Three entry double array. result[ 0 ] = sum of counts result[ 1 ]
	 *         = sum of squared counts result[ 2 ] = unique count (size of map)
	 */
	...
	public static double getSumOfCrossProducts(
		...
		if (countMap1.size() > countMap2.size()) {
			for (Iterator<?> iterator = countMap2.keySet().iterator(); iterator
					.hasNext();) {
			...
			for (Iterator<?> iterator = countMap1.keySet().iterator(); iterator
					.hasNext();) {
				Object key = iterator.next();
	 ...
	 *            The count map.
	 * @param scaleFactor
	 *            The double value by which to multiply each count value in the
	 *            count map.
	 * 
	 * @return Map with same keys as input map and counts scaled using
	 *         scaleFactor.
	 */

	public static <K> Map<K, Number> scaleCountMap(
			Map<K, ? extends Number> map, double scaleFactor) {
		Map<K, Number> result = new TreeMap<K, Number>();
	 ...
	 * 
	 * @param map
	 *            The map with arbitrary objects as keys and Number counts as
	 *            values.
	 * 
	 ...
	 * 
	 *            <p>
	 *            On output, the destination map is updated with words and
	 *            counts from the source map. The key type for both input maps
	 *            must be the same for this to make sense.
	 *            </p>
	 */

	public static <K> void addCountMap(Map<K, Number> destinationMap,
		...
		while (iterator.hasNext()) {
			...
			if (!destinationMap.containsKey(key)) {
				destinationMap.put(key, NumberOps.add(sourceCount, new Double(
						0.0D)));
			} else {
	 ...
	 * 
	 *            <p>
	 *            On output, the destination map counts are incremented by one
	 *            for each word appearing in the source map. If a source word
	 *            does not already appear in the destination, it is added with a
	 *            count of one.
	 *            </p>
	 */

	public static <K> void incrementCountMap(Map<K, Number> destinationMap,
			Map<K, Number> sourceMap) {
		Iterator<K> iterator = sourceMap.keySet().iterator();
		...
		while (iterator.hasNext()) {
			...
			if (!destinationMap.containsKey(key)) {
				...
				destinationMap.put(key, NumberOps
						.add(destCount, new Integer(1)));
			}
		}
	}
	 ...
	 * 
	 *            <p>
	 *            On output, the destination map counts are updated by removing
	 *            the counts for matching words from the source map. If the
	 *            count goes to zero for any word in the destination, that word
	 *            is removed from from the destination map.
	 *            </p>
	 */

	public static <K> void subtractCountMap(Map<K, Number> destinationMap,
			Map<K, Number> sourceMap) {
		Iterator<K> iterator = sourceMap.keySet().iterator();
		...
		while (iterator.hasNext()) {
			...
			if (destinationMap.containsKey(key)) {
				...
				Number updatedDestCount = NumberOps.subtract(destCount,
						sourceCount);
	 ...
	 *            Second count map.
	 * 
	 * @return Count of words appearing in both maps. The key type for the two
	 *         maps should be the same for this to make sense.
	 */
	 ...
	 *            The word text.
	 * 
	 * @return The count for the specified word as an integer, 0 if the word
	 *         does not occur.
	 */
	...
	public static <K> void updateWordCountMap(K word, int count,
			Map<K, Number> countMap) {
		Number newCount;

		if (countMap.containsKey(word)) {
			newCount = NumberOps.add(countMap.get(word), new Integer(count));
		} else {
	 ...
	 * 
	 * @param countMap
	 *            The count map whose keys should be converted to strings.
	 * 
	 * @return The count map with keys converted to plain strings.
	 * 
	 *         <p>
	 *         Each key element's toString() method is called to convert the key
	 *         object to a plain text string. Key elements without a toString()
	 *         method will not be added to the result map. The object values
	 *         (counts) are left untouched. Note that no key should be null.
	 ...
	 * 
	 *         <p>
	 *         Each line of the input file has one string, followed by an Ascii
	 *         tab character, followed by an integer count.
	 *         </p>
	 */
	 ...
	 *         <p>
	 *         Each line of the input file has one string, followed by an Ascii
	 *         tab character, followed by a count. The counts may be integers or
	 ...
	 *         count value and creating new string keys using "toString()" on
	 *         the original map's key values. For a count map with string keys,
	 *         this produces a deep clone. For a count map with non-string keys,
	 *         this produces what might be called a semi-deep clone, since only
	 *         the string values of the original keys are duplicated.
	 *         </p>
	 */

	public static <K, V extends Number> Map<String, Number> semiDeepClone(
			Map<K, V> countMap) {
		// Create a map to hold the
		// results of the conversion.
