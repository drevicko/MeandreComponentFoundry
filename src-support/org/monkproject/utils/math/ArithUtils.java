 ...
 * 
 * <p>
 * The following methods for trigonometric functions come from the Sfun class
 * written by Visual Numerics.
 * </p>
 * 
 * <ul>
 * <li>acosh</li>
...
public class ArithUtils {
	 ...
	 * 
	 *         <p>
	 *         This method is a modified version of the one in the Visual
	 *         Numerics Sfun class.
	 *         </p>
	 */
	...
	public static double asinh(double x) {
		...
		// 1.05367e-08 = Math.sqrt(EPSILON_SMALL)
			...
			ans = x
					* (1.0D + Polynomial.evaluateChebyschev(ASINH_COEF, 2.0D
							* x * x - 1.0D));
		}
	...
	public static double atanh(double x) {
		...
		// 1.82501e-08 = Math.sqrt(3.0*EPSILON_SMALL)
			...
			ans = x
					* (1.0D + Polynomial.evaluateChebyschev(ATANH_COEF, 8.0D
							* x * x - 1.0D));
		} else if (y < 1.0D) {
	...
	public static double cot(double x) {
		...
		// 1.82501e-08 = Math.sqrt(3.0*EPSILON_SMALL)
			...
			ans = (0.5D + Polynomial.evaluateChebyschev(COT_COEF, 32.0D * y * y
					- 1.0D))
					/ y;
		}

		else if (y <= 0.5D) {
			ans = (0.5D + Polynomial.evaluateChebyschev(COT_COEF, 8.0D * y * y
					- 1.0D))
					/ (0.5D * y);
			...
			ans = (0.5D + Polynomial.evaluateChebyschev(COT_COEF, 2.0D * y * y
					- 1.0D))
					/ (0.25D * y);
	 ...
	 * 
	 * @param x
	 *            The number for which to find exp( x ) - 1.
	 * 
	 * @return exp( x ) - 1.
	 * 
	 *         <p>
	 ...
	 * 
	 *         <p>
	 *         This is an implementation of an algorithm suggested by Donald E.
	 *         Knuth in Section 4.2.2 of
	 ...
	 * 
	 *         <p>
	 *         The hypotenuse value is given mathematically as the sqrt( a^2 +
	 *         b^2 ). The method implemented here reduces the chances of
	 *         cancellation and roundoff error. If the |a| > |b|, we compute the
	 *         hypotenuse as:
	 *         </p>
	 * 
	 *         <p>
	 ...
	 * 
	 * @return The log base 2 of x. If x is <= 0, 0 is returned.
	 * 
	 *         <p>
	 ...
	 * 
	 * @return The log base 10 of x. If x is <= 0, 0 is returned.
	 * 
	 *         <p>
	 ...
	 * 
	 * @param x
	 *            The number for which we want log( x + 1 ).
	 * 
	...
	public static double logSumLogs(double logX, double logY) {
		return Math.max(logX, logY) + log1p(Math.exp(-Math.abs(logX - logY)));
	}
	 ...
	 * 
	 * @return The natural log of x. If x is zero, returns 0.
	 */
	...
	public static double sinh(double x) {
		...
		// 2.58096e-08 = Math.sqrt( 6.0 * EPSILON_SMAL L)
			...
			ans = x
					* (1.0D + Polynomial.evaluateChebyschev(SINH_COEF, 2.0D * x
							* x - 1.0D));
		}
	...
	public static double tanh(double x) {
		...
		// 1.82501e-08 = Math.sqrt(3.0*EPSILON_SMALL)
			...
			ans = x
					* (1.0D + Polynomial.evaluateChebyschev(TANH_COEF, 2.0D * x
							* x - 1.0D));
		}
