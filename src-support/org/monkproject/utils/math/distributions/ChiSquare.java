...
public class ChiSquare {
	 ...
	 * 
	 * @param chisq
	 *            Percentage point of chi-square distribution
	 * @param df
	 *            Degrees of freedom
	 * 
	 * @return The corresponding probabiity for the chi-square distribution.
	 * 
	 *         <p>
	 *         The probability is determined using the relationship between the
	 *         incomplete gamma CDF and the chi-square distribution:
	 *         </p>
	 * 
	 *         <p>
	 ...
	 * 
	 * @param p
	 *            Probability level for which to compute percentage point
	 * 
	 * @param df
	 *            Degrees of freedom
	 * 
	 * @return The corresponding percentage for the chi-square distribution.
	 * 
	 * @throws IllegalArgumentException
	 *             If df <= 0 or p very near zero or p very near one. A
	 *             tolerance of 1.0e-15 is used for testing the value of p.
	 * 
	 *             <p>
	 *             Implements the method of Best and Roberts (1975). The result
	 *             accuracy varies, but should be good to at least 10 decimal
	 *             digits in all cases.
	 *             </p>
	 */
	...
	public static double chisquareInverse(double p, double df) {
		...
		if (df < (-1.24D * Math.log(p))) {
			ch = Math.pow(p * xx * Math.exp(g + xx * Constants.LN2),
					(1.0D / xx));
		...
		/* Starting approx. for df <= .32 */
			...
			do {
				...
				t = -0.5D + (4.67D + 2.0D * ch) / p1
						- (6.73D + ch * (13.32D + 3.0D * ch)) / p2;

				ch = ch
						- (1.0D - Math.exp(a + g + 0.5D * ch + c
								* Constants.LN2)
								* p2 / p1) / t;
		...
		do {
			...
			t = p2 * Math.exp(xx * Constants.LN2 + g + p1 - c * Math.log(ch));
			...
			s1 = (210.0D + a
					* (140.0D + a
							* (105.0D + a * (84.0D + a * (70.0D + 60.0D * a))))) / 420.0D;

			s2 = (420.0D + a
					* (735.0D + a * (966.0D + a * (1141.0D + 1278.0D * a)))) / 2520.0D;

			s3 = (210.0D + a * (462.0D + a * (707.0D + 932.0D * a))) / 2520.0D;

			s4 = (252.0D + a * (672.0D + 1182.0D * a) + c
					* (294.0D + a * (889.0D + 1740.0D * a))) / 5040.0D;
