...
public class FishersF {
	 ...
	 * 
	 * @param f
	 *            Percentage point of Fisher's F distribution
	 * @param dfn
	 *            Numerator degrees of freedom
	 * @param dfd
	 *            Denominator degrees of freedom
	 * 
	 * @return The corresponding probabiity for Fisher's F distribution.
	 * 
	...
	public static double f(double f, double dfn, double dfd)
		...
		if ((dfn > 0.0D) && (dfd > 0.0D)) {
			result = Beta.incompleteBeta(dfd / (dfd + f * dfn), dfd / 2.0D,
					dfn / 2.0D, Constants.MAXPREC);
		} else {
	...
	public static double fInverse(double p, double dfn, double dfd)
		...
		if ((dfn > 0.0D) && (dfd > 0.0D)) {
			if ((p >= 0.0D) && (p <= 1.0D)) {
				result = Beta.incompleteBetaInverse(1.0D - p, dfn / 2.0D,
						dfd / 2.0D);
