...
public class Normal {
	 ...
	 * 
	 * @return The corresponding probabiity for the Student t distribution.
	 * 
	 *         <p>
	...
	public static double normal(double z) {
		...
		if (z >= 0.0D) {
			result = (1.0D + ErrorFunction.errorFunction(z / Constants.SQRT2)) / 2.0D;
		} else {
			result = ErrorFunction
					.errorFunctionComplement(-z / Constants.SQRT2) / 2.0D;
		}
	 ...
	 *            Probability value.
	 * 
	 * @return The corresponding approximate percentage point for the normal
	 *         distribution.
	 * 
	 *         <p>
	 *         See Wichura, M. J. (1988) Algorithm AS 241: The Percentage Points
	 *         of the Normal Distribution. Applied Statistics, 37, 477-484. The
	 *         result is generally accurate to about 10-12 decimal digits. We
	 *         improve the result from Wichura's estimate using two iterations
	 *         of a Taylor series, generally resulting in about 15 decimal
	 *         digits of accuracy. See Kennedy, W. J. and Gentle, James E.
	 *         _Statistical Computing_, Marcel Dekker, 1980, pp. 94 for a
	 *         discussion of the Taylor series improvement.
	 *         </p>
	 */

	public static double normalInverse(double p)
			throws IllegalArgumentException {
		final double a[] = { 3.3871328727963666080e0, 1.3314166789178437745e+2,
				1.9715909503065514427e+3, 1.3731693765509461125e+4,
				4.5921953931549871457e+4, 6.7265770927008700853e+4,
				3.3430575583588128105e+4, 2.5090809287301226727e+3 };

		final double b[] = { 1.0000000000000000000e0, 4.2313330701600911252e+1,
				6.8718700749205790830e+2, 5.3941960214247511077e+3,
				2.1213794301586595867e+4, 3.9307895800092710610e+4,
				2.8729085735721942674e+4, 5.2264952788528545610e+3 };

		final double c[] = { 1.42343711074968357734e0,
				4.63033784615654529590e0, 5.76949722146069140550e0,
				3.64784832476320460504e0, 1.27045825245236838258e0,
				2.41780725177450611770e-1, 2.27238449892691845833e-2,
				7.74545014278341407640e-4 };

		final double d[] = { 1.00000000000000000000e0,
		...
		final double e[] = { 6.65790464350110377720e0,
				5.46378491116411436990e0, 1.78482653991729133580e0,
				2.96560571828504891230e-1, 2.65321895265761230930e-2,
				1.24266094738807843860e-3, 2.71155556874348757815e-5,
				2.01033439929228813265e-7 };

		final double f[] = { 1.00000000000000000000e0,
		...
		if (p > 1.0) {
			...
			if (Math.abs(q) <= SPLIT1) {
				double r = CONST1 - q * q;
				z = q * Polynomial.hornersMethod(a, r)
				...
				if (r <= SPLIT2) {
					r -= CONST2;
					z = Polynomial.hornersMethod(c, r)
							/ Polynomial.hornersMethod(d, r);
				} else {
					r -= SPLIT2;
					z = Polynomial.hornersMethod(e, r)
			...
			for (int i = 0; i < 2; i++) {
				...
				double phi = Math.sqrt(1.0 / (2.0 * Math.PI))
						* Math.exp(-(z * z) / 2.0);
