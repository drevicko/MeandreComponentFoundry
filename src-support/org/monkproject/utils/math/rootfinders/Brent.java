 ...
 * 
 * <p>
 * Brent's method requires as input an initial interval [x0,x1] which brackets
 * one (or an odd number) of roots.
 * </p>
 * 
 * <p>
 * During each iteration Brent's method constructs an interpolation curve to
 * approximate the function. For the initial iteration this is a simple linear
 * interpolation using the initially supplied endpoints of the interval
 * bracketing the root. For subsequent iterations the interpolation curve is an
 * inverse quadratic fit to the last three points. The intercept of the
 * interpolation curve with the x-axis provides the root approximation. If this
 * value lies within the bounds of the current root bracketing interval the
 * interpolation point is accepted and used to narrow the interval. If the
 * interpolated value lies outside the bounds of the current interval the
 * algorithm switches to bisection steps to narrow the interval. The algorithm
 * switches between quadratic interpolation and bisection as often as necessary
 * to ensure convergence.
 * </p>
 * 
 * <p>
 * The best estimate of the root is taken from the most recent interpolation or
 * bisection after the value is found to a specified tolerance or a specified
 * number of iterations is exhausted.
 * </p>
 * 
 * <p>
 * See Brent, Richard P. <strong>Algorithms for minimization without
 * derivatives.</strong> Mineola, N.Y. : Dover Publications, 2002.
 * </p>
 * <p>
 * and
 * </p>
 * <p>
 * Brent, R. P. <em>An algorithm with guaranteed convergence for finding
 * 	the zero of a function.</em> <strong>The Computer Journal</strong> vol. 14,
 * no. 4, pp. 422-425.
 * </p>
 * 
 * <p>
 * This Java code is a fairly straightforward translation of Brent's original
 * Algol 60 version as published in his paper in The Computer Journal.
 * </p>
 */
...
public class Brent implements MonadicFunctionRootFinder {
	 ...
	 * 
	 * @param tol
	 *            Convergence tolerance to which root is computed.
	 * 
	 * @param maxIter
	 *            Maximum number of iterations.
	 * 
	 * @param function
	 *            The function whose root to find.
	 * 
	 ...
	 *             when [x0,x1] does not contain root and the attempt to expand
	 *             the interval to contain a root fails.
	 * 
	 *             <p>
	...
	public static double brent(double x0, double x1, double tol, int maxIter,
		...
		for (iter = 1; iter <= maxIter; iter++) {
			...
			if (iterationInformation != null) {
				iterationInformation.iterationInformation(b, fb, Double.NaN,
						iter);
			}
			 ...
			 */
			if (convergenceTest.converged(xm, 0.0D, fb, tol1, 0.0D))
				return b;
	...
	public static double brent(double x0, double x1, double tol, int maxIter,
		...
		return brent(x0, x1, tol, maxIter, function,
	 ...
	 *             <p>
	 *             Function must have an odd # of roots in the interval [x0,x1]
	 *             . Up to 100 iterations are attempted with a convergence
	 *             tolerance set to Constants.MACHEPS .
	 *             </p>
	 */

	public static double brent(double x0, double x1, MonadicFunction function)
			throws IllegalArgumentException {
		return brent(x0, x1, Constants.MACHEPS, 100, function,
				new StandardRootFinderConvergenceTest(), null);
	}
	...
	public double findRoot(double x0, double x1, double tol, int maxIter,
			...
			RootFinderIterationInformation iterationInformation)
			throws IllegalArgumentException {
		return brent(x0, x1, tol, maxIter, function, convergenceTest,
